...
I am not an epidemiologist or a health official. I am a Biomedical Engineer getting a PhD in Immunology from Duke, and an amateur Data Scientist. Conclusions made within the project, are not be considered as fact.
...
```
These data contain information for 2,896 counties over 115 days startin in January, and ending in mid-May. The first confirmed COVID-19 related death was widely considered to be on Feb.29th, but it was later determined to have been as early as Feb. 6th [Source](https://www.nbcnews.com/news/us-news/first-u-s-coronavirus-death-happened-weeks-earlier-originally-believed-n1189286).
...
```
This visual shows where the hotspots are in a more granular view. Additionally it is clear that there are many counties that haven't experienced any deaths yet. 
...
Surprisingly, over 40 percent of the counties with data have yet to have a COVID related death. This is improtant for model building, because if we try to predict deaths, roughly 60 percent of the data will not have a death outcome to predict. 
...
```
## Outcome Definition
Because the county data has more observations, as there are more counties than states, we will define our outcome using this variation of the dataset.Below is a visualization of how COVID-19 deaths have grown over time in counties that have documented more than 10 COVID-19 deaths. For the purpose of this project, less than 10 deaths in a county over 115 days observed will be considered negligible growth. 
...
```
From this visual we can conclude that around 20 days after a conties 10th death, many of the curves levelled off. Interestingly, many of the log(death) curves are  mostly linear for the first 14 days. 
...
red_covid %>%
  ...
  ggtitle("COVID19 deaths by US county")
```
This suggests that we can perform linear regression, and the slope coeffecient of the fitted line will describe the growth rate for each county, with higher slopes meanign faster COVID-19 fatality growth. 
...
slopes %>% ggplot(aes(slope)) + geom_histogram() + geom_vline(xintercept =  mu, col = "red") +
  geom_text(aes(x=mu*1.025, label="Average", y=60), colour="red", angle=90) +
...
sum(nzv$nzv)
```
Fortunately, none of the predictors have nearly zero variance, so we will maintain them all. 
...
#Showing summary of correlation
  ...
  kable_styling(latex_options =c("striped"),font_size = 11)
```
This can also be visualized in a heatmap.
```{r cor-heatmap}
...
```
Considering this, a cutoff for being considered highly correlated will be set at .75. 

A dataset is created that has the highly correlated features removed.
...
```{r corr-out-distribution}
#plotting correlation ditribution
corrs2 %>% ggplot(aes(corrs)) + geom_histogram() +
  ggtitle("Distribution of Correlations of Features") +
  ylab("Count") + xlab("Correlation Values")
```
Additionally the heatmap of correlations looks more symetrical.
```{r cor-out-heatmap}
...
### Principle COmponenta Analysis - PCA
PCA seeks to reduce dimensionality of data by ensuring that the new features - called principle components (PCs) - are orthagonal to each other and thus should be independent and uncorrelated. PCA is a linear algorithm.
...
sum_pca_w_cors$importance[,1:2] %>% 
  ...
  kable_styling(latex_options =c("striped"),font_size = 11)
```
It appears that the first two PCs do not adequately seperate the groups. Although it appears the high growth counties have little overlap with the counties that experienced little to no growth. We can see that the first two principle components only explain about 50 percent of the total variance in the data. 

If the highly correlated variables are removed, this may improve the visualisation of the data. Below is the PC analysis plot of that data. 
```{r cor-out-pca}
...
sum_pca_wo_cors$importance[,1:2] %>% 
  ...
  kable_styling(latex_options =c("striped"),font_size = 11)
```
Here we see more overlap with the the all the growth categories, this suggests the data without the correlated features will have worse ability to resolve differences in groups. The first two PCs of this dataset explain only 30 percent. Further demonstrating that removing correlated variables might reduce the ability of the data to resolve the growth categories. 

### t-SNE
T-distributed Stochastic Neighbor Embedding (t-SNE) is a non-linear dimensionality reduction method. t-SNE, like kNN, uses Euclidian distance but instead converts that distance to a conditional probablity. This probability is the likelihood that a given value would select another value to be close to it. The formula for this conditional probabilty is below:

$$p_{j|i}=\frac{exp(-||x_i-x_j||^2/2\sigma_i^2)}{\sum_{k\neq i} exp(-||x_i-x_j||^2/2\sigma_i^2)}$$
Below is the t-SNE plot for the data that includes the correlated features, as this was the dataset that clustered better with PCA.
```{r t-sne}
...
## Modelling
This section will focus on using machine learning algorithms to idenify socio-economic and health predictors that are imporant for classifying the the degree of growth experienced in the early days of COVID-19.
### Data Partitioning
...
results %>% kable(.,"latex",booktabs = T) %>% 
  kable_styling(latex_options =c("striped"),font_size = 11)
```
This shows that for this data, the Yeo-Johnson transformed data improves the prediction, while scaling the data does not. 

The Yeo-Johnson transformed data will be used moving forward for the rest of model training. Using this transformed data, the k-nearest neighbor will be tuned, to maximize accuracy. 
...
#yj transform
yj_pre_proc_out <- preProcess(edx_no_cor[,-ncol(edx_no_cor)],method = c("YeoJohnson"))
yj_edx_cor_out <- predict(yj_pre_proc_out,edx_no_cor)
...
```
The k-tuning on the data without highly correlated features, seems to converge instead of peak. The performance of the kNN algorithm on the data without highly correlated features performs worse than that the comlete training data. 
...
impvars_knn %>% kable(.,"latex",booktabs = T) %>% 
  kable_styling(latex_options =c("striped"),font_size = 11)
```
The top 10 important features, roughly describe urban areas. From our previous covid growth plots displayed on maps, we know that this is often where the growth was the highest. It is interesting to seethese feaures identified by a machine learning algorithm, even if some of them may have been intuitive. 

We will confirm these feaures with a second machine learning model to determine the there is consensus on what factors are important between algorithms. 
...
rpart_no_cor_fit <- train(fct_slope ~ ., data = yj_edx_cor_out,
                          method = "rpart",
                          trControl = fitControl,
...
#storing results 
results <- results %>% add_row(Method = "YJ Tuned CART",
                               Data = "Training w/o Corr Features", 
...
impvars %>% kable(.,"latex",booktabs = T) %>% 
  kable_styling(latex_options =c("striped"),font_size = 11)
```
The important variables in both models are largely overalapping, and the interpretation is still that cities tend to see higher growth than more rural areas. 
### Model Validation
Finally, the Yeo-Johnson transformed, normalized kNN model with correlates will be used to predict the growth category for each county in the validation dataset. 
